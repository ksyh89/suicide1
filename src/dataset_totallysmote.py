...
class Dataset(data.Dataset):
    ...
    def convert_and_save(self, processed_datapath):
        ...
        train_input, train_label = smote.fit_resample(df.iloc[:, 1:], df.iloc[:, :1])
    ...
    def fillempty(self, dataframe, column_type):

    #Smote위해 추가

        for i, column_name in enumerate(dataframe.columns):
            c_type = column_type[column_name]
            is_categorical = self.is_categorical(c_type, column_name)

            if is_categorical:
                #print(dataframe[column_name])
                #dataframe[column_name]=dataframe[column_name].fillna(-9999.0, inplace=True)
                values = {column_name : -9999.0}
                ...
                dataframe.fillna(value = values, inplace = True)
                #print(dataframe[column_name])
                #dataframe[column_name]=dataframe[column_name].fillna(0.0, inplace=True)
